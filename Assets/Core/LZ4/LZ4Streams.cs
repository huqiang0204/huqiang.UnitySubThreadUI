...
namespace huqiang.LZ4
{
    ...
    public class LZ4Streams
    {
        ...
        public static void DecompressFolder(Stream fs, string folder)
        {
            byte[] tmp = new byte[4];
            int len = (int)fs.Length;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else if (o > 0)
                {
                    byte[] tar = new byte[o];
                    fs.Read(tar, 0, o);
                    WriteFile(folder, f, tar, o);
                }
                else
                {
                    WriteFile(folder, f, null, 0);
                }
            }
        }
